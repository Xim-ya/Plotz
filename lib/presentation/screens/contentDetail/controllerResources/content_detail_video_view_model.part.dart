...
extension ContentDetailVideoViewModel on ContentDetailViewModel {
  ...
  bool get isVideoContentLoaded {
    if (contentType.isMovie) {
      return oldContentVideos?.isDetailInfoLoaded ?? false;
    } else {
      if (oldContentVideos?.contentVideoFormat ==
          ContentVideoFormat.multipleTv) {
        return oldContentVideos?.isSeasonInfoLoaded ?? false;
      } else {
        return oldContentVideos?.isDetailInfoLoaded ?? false;
      }
    }
  ...
  String? get singleVideoThumbnailUrl =>
      oldContentVideos?.singleTypeVideo.detailInfo?.videoThumbnailUrl;

  String? get singleVideoId =>
      passedArgument.videoId ?? oldContentVideos?.singleTypeVideo.videoId;

  // 유튜브 컨텐츠 조회수
  String? get singleVideoViewCount => Formatter.formatNumberWithUnit(
        oldContentVideos?.singleTypeVideo.detailInfo?.viewCount,
        isViewCount: true,
      );

  // 유튜브 컨텐츠 좋아요 수
  String? get singleLikesCount => Formatter.formatNumberWithUnit(
      oldContentVideos?.singleTypeVideo.detailInfo?.likeCount,);

  // 유튜브 컨텐츠 업로드 일자
  String? get singleUploadDate => Formatter.getDateDifferenceFromNow(
      oldContentVideos?.singleTypeVideo.detailInfo?.uploadDate,);

  /* [ContentVideoFormat] - multiple 타입 리소스  */

  // 멀티 비디오 객체
  List<OldContentVideoItem>? get multipleVideoInfo =>
      oldContentVideos?.multipleTypeVideos;
}
